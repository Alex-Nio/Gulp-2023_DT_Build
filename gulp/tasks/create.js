...
export const createComponent = () => {
  ...
  const componentsScss = fs.readFileSync("src/styles/scss/config/base/components.scss", "utf8");
  ...
  fs.writeFileSync("src/styles/scss/config/base/components.scss", updatedComponentsScss);

  file(`${componentName}.scss`, "", { src: true })
    .pipe(gulp.dest(`src/html/components/${componentName}`))
    .pipe(inject.prepend(myComponentScss))
    .pipe(appendPrepend.appendFile("src/styles/scss/config/base/components.scss")); // Use appendFile method
